...
process_execute(const char *file_name)//执行线程，file_name就是输入的包括参数的一长串指令
{
  ...
  real_name = strtok_r((char *)file_name, " ", &save_ptr); //要分割的字符串，分隔符，用于保存上下文的指针变量
  /*添加*/
...
/* A thread function that loads a user process and starts it
   running. */
static void
start_process(void *file_name_)//file_name包括了参数，所以要在此对参数进行处理
{
  ...
  token = strtok_r((char*)file_name, " ", &save_ptr);//参数分离
  // printf("start:%s\n", token);
  // printf("args:%s\n", strlen(token));
  /*添加*/
  /* Initialize interrupt frame and load executable. */
  memset(&if_, 0, sizeof if_);//寄存器初始化
...
void process_exit(void)
{
  ...
  /* Destroy the current process's page directory and switch back
     to the kernel-only page directory. 用户进程的页不为空，内核进程的页为空 */
  pd = cur->pagedir;
  if (pd != NULL)//如果是用户线程
  {
...
validate_segment(const struct Elf32_Phdr *phdr, struct file *file)
{
  ...
  if (phdr->p_vaddr < PGSIZE)//p_vaddr->p_offset
    return false;
